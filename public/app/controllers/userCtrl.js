...
// Controller: canvasCtrl is used finalize facebook login   
.controller('canvasCtrl', function($routeParams, Auth, $location, $window, $scope) {
        ...
        // Check if callback was successful         
        if ($window.location.pathname == '/canvaserror') {
            $scope.alert = 'alert alert-danger'; // Set class for message
            app.errorMsg = 'sorry something broke.'; // If error, display custom message
        } else if ($window.location.pathname == '/canvas/inactive/error') {
            app.expired = true; // Variable to activate "Resend Link Button"
            $scope.alert = 'alert alert-danger'; // Set class for message
            app.errorMsg = 'Account is not yet activated. Please check your e-mail for activation link.'; // If error, display custom message
        } else {
